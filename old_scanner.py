...
class Scanner:
    ...
    def backtracking_search_helper(self, matrix, idx):
        ...
        element = matrix_flattened[idx]

        # skip assignment if element is fixed
        if element.is_fixed:
            return self.backtracking_search_helper(matrix, idx+1)

        # else, assign all possible values
        matrix_left = deepcopy(matrix_flattened)
        matrix_right = deepcopy(matrix_flattened)
        matrix_left[idx].value = False
        matrix_right[idx].value = True
    ...
    def backtracking_search(self):
        element = self.matrix[0,0]
        matrix_left = deepcopy(self.matrix)
        matrix_right = deepcopy(self.matrix)
        if not element.is_fixed:
            matrix_left[0,0].value = False
            matrix_right[0,0].value = True
        return self.backtracking_search_helper(matrix_left, 1) or self.backtracking_search_helper(matrix_right, 1)

    def _pretty_str(self, matrix):
        str = ""
        for row in np.vectorize(lambda b: ' ' if b.value == None else '#' if b.value else '.')(matrix):
            for char in row:
                print(char, end="")
            print()
        return str
    ...
    def __str__(self):
        return self._pretty_str(self.matrix)
